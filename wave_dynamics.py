...
class WaveDynamicsIndicator:
    ...
    def compute_signal_engine(
        ...
        session_allowed = session_mask(df.index, self.config['i_asiaStart']) if self.config['i_enableSession'] else pd.Series(True, index=df.index)
    ...
    def compute_channel_metrics(
        ...
        if len(high_indices) >= 2 and len(low_indices) >= 2:
            ...
            upper = swing_high.iloc[h2] + high_slope * (t - t[h2])
            lower = swing_low.iloc[l2] + low_slope * (t - t[l2])
            valid = (upper != lower)
    ...
    def run(self, df: pd.DataFrame) -> Dict[str, object]:
        ...
        if self.config['i_showWaveProjections'] and wave_sequence:
            ...
            wave4_mag = wave_sequence.get("wave4_mag")
            avg_mag = np.nan
            ...
            if avg_mag and not np.isnan(avg_mag):
                ...
                measured_move.iloc[-1] = last_close + avg_mag * 0.786
        return {
            "atr": atr_series,
            "lookback": lookback,
