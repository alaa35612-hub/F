...
class WaveDynamicsIndicator:
    ...
    def compute_signal_engine(
        ...
        filters_pass_long = (~is_chop_zone) & session_allowed
        filters_pass_short = (~is_chop_zone) & session_allowed
    ...
    def run(self, df: pd.DataFrame) -> Dict[str, object]:
        ...
        if self.config['i_showWaveProjections'] and wave_sequence:
            wave1_mag = wave_sequence.get("wave1_mag")
            wave3_mag = wave_sequence.get("wave3_mag")
            avg_mag = np.nan
            if wave1_mag and wave3_mag:
                avg_mag = (wave1_mag + (wave3_mag if not np.isnan(wave3_mag) else wave1_mag)) / 2
            if avg_mag and not np.isnan(avg_mag):
                measured_move.iloc[-1] = df['close'].iloc[-1] + avg_mag * 0.786
        trades = self.compute_trade_management(df, atr_series, signal_engine)
        return {
            "atr": atr_series,
            "lookback": lookback,
