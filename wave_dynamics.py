...
_CCXT_SPEC = importlib.util.find_spec("ccxt")
if _CCXT_SPEC is not None:
    import ccxt
else:
    ccxt = None
...
CONFIG: Dict[str, object] = {
    ...
    'i_scanSignalAge': 3,
    'i_scanMaxSymbols': 50,
    'i_scanMinQuoteVolume': 0.0,
    'i_scanRateLimitMs': 250,
    'i_plotEnabled': False,
}
...
def percentile_linear(series: pd.Series, length: int, percentile: float) -> pd.Series:
...
def scan_binance_usdm(config: Dict[str, object]) -> Dict[str, List[str]]:
    if ccxt is None:
        raise RuntimeError("CCXT is not installed. Install ccxt to enable Binance scanning.")
    ...
    max_symbols = int(config['i_scanMaxSymbols'])
    if max_symbols > 0:
    ...
    buy_hits = []
    sell_hits = []
    signal_age = int(config['i_scanSignalAge'])
    limit = int(config['i_scanLimit'])
    timeframe = str(config['i_scanTimeframe'])
    min_quote_volume = float(config['i_scanMinQuoteVolume'])

    for symbol in symbols:
        if min_quote_volume > 0:
            try:
                ticker = exchange.fetch_ticker(symbol)
                if ticker.get("quoteVolume", 0.0) < min_quote_volume:
                    continue
            except Exception:
                continue
        try:
        ...
        buy_indices = buy_signal[buy_signal].index
        sell_indices = sell_signal[sell_signal].index
        if len(buy_indices) > 0:
            last_buy_idx = df.index.get_loc(buy_indices[-1])
            if len(df) - 1 - last_buy_idx <= signal_age:
                buy_hits.append(symbol)
        if len(sell_indices) > 0:
            last_sell_idx = df.index.get_loc(sell_indices[-1])
            if len(df) - 1 - last_sell_idx <= signal_age:
                sell_hits.append(symbol)
        if config['i_scanRateLimitMs']:
            exchange.sleep(config['i_scanRateLimitMs'])
    return {"buy": buy_hits, "sell": sell_hits}
...
class WaveDynamicsIndicator:
    ...
    def compute_ribbon_metrics(self, df: pd.DataFrame, atr_series: pd.Series) -> Dict[str, pd.Series]:
        ribbon = self.compute_ribbon(df['close'], atr_series)
        vwap_like = vwma(df['close'], df['volume'], 20)
        true_mean = (ribbon[21] + ribbon[55] + vwap_like) / 3.0
        ribbon_top = pd.concat([ribbon[8], ribbon[13], ribbon[21], ribbon[34], ribbon[55]], axis=1).max(axis=1)
        ribbon_bot = pd.concat([ribbon[8], ribbon[13], ribbon[21], ribbon[34], ribbon[55]], axis=1).min(axis=1)
        ribbon_mid = (ribbon_top + ribbon_bot) / 2.0
        ribbon_width = ribbon_top - ribbon_bot
        ribbon_normal_width = sma(ribbon_width, 50)
        is_ribbon_tight = ribbon_width < ribbon_normal_width * 0.7
        is_ribbon_wide = ribbon_width > ribbon_normal_width * 1.3
        ribbon_state = np.where(is_ribbon_tight, "COMPRESSED", np.where(is_ribbon_wide, "EXPANDED", "NORMAL"))
        bullish_stack = (ribbon[8] > ribbon[13]) & (ribbon[13] > ribbon[21]) & (ribbon[21] > ribbon[34]) & (ribbon[34] > ribbon[55])
        bearish_stack = (ribbon[8] < ribbon[13]) & (ribbon[13] < ribbon[21]) & (ribbon[21] < ribbon[34]) & (ribbon[34] < ribbon[55])
        strict_bull_stack = bullish_stack & (ribbon[55] > ribbon[89]) & (ribbon[89] > ribbon[144])
        strict_bear_stack = bearish_stack & (ribbon[55] < ribbon[89]) & (ribbon[89] < ribbon[144])
        premium_zone = df['close'] > true_mean + atr_series * 0.5
        discount_zone = df['close'] < true_mean - atr_series * 0.5
        return {
            "ribbon": ribbon,
            "true_mean": true_mean,
            "ribbon_top": ribbon_top,
            "ribbon_bot": ribbon_bot,
            "ribbon_mid": ribbon_mid,
            "ribbon_state": pd.Series(ribbon_state, index=df.index),
            "bullish_stack": bullish_stack,
            "bearish_stack": bearish_stack,
            "strict_bull_stack": strict_bull_stack,
            "strict_bear_stack": strict_bear_stack,
    ...
    def track_wave_sequence(
        ...
        active_series = np.zeros(len(df))
        w4_end_price_series = np.full(len(df), np.nan)
        ...
        for i in range(len(df)):
            ...
            active_series[i] = 1 if in_active_sequence else 0
            w4_end_price_series[i] = w4_end_price

        return {
            "wave_count": pd.Series(wave_count_series, index=df.index),
            "sequence_dir": pd.Series(sequence_dir_series, index=df.index),
            "in_active_sequence": pd.Series(active_series, index=df.index).astype(bool),
            "wave1_mag": wave1_mag,
            "wave2_mag": wave2_mag,
            "wave3_mag": wave3_mag,
            "wave4_mag": wave4_mag,
            "wave5_mag": wave5_mag,
            "w4_end_price": pd.Series(w4_end_price_series, index=df.index),
        }

    def compute_structure(self, df: pd.DataFrame, swing_high: pd.Series, swing_low: pd.Series, relative_volume: pd.Series) -> Dict[str, pd.Series]:
        ...
        market_structure = pd.Series("RANGING", index=df.index)
        for i in range(len(df)):
            if not np.isnan(swing_high.iloc[i]):
                last_swing_high = swing_high.iloc[i]
            if not np.isnan(swing_low.iloc[i]):
                last_swing_low = swing_low.iloc[i]
            bull_bos_raw = (
                self.config['i_enableChoch']
                and not np.isnan(last_swing_high)
                and df['close'].iloc[i] > last_swing_high
                and (df['close'].iloc[i - 1] <= last_swing_high if i > 0 else False)
                and relative_volume.iloc[i] >= 1.0
            )
            bear_bos_raw = (
                self.config['i_enableChoch']
                and not np.isnan(last_swing_low)
                and df['close'].iloc[i] < last_swing_low
                and (df['close'].iloc[i - 1] >= last_swing_low if i > 0 else False)
                and relative_volume.iloc[i] >= 1.0
            )
            if bull_bos_raw and (i - last_bull_bos_bar > self.config['i_chochCooldown']):
                bull_bos.iloc[i] = True
                last_bull_bos_bar = i
            if bear_bos_raw and (i - last_bear_bos_bar > self.config['i_chochCooldown']):
                bear_bos.iloc[i] = True
                last_bear_bos_bar = i
            if bull_bos.iloc[i]:
                market_structure.iloc[i] = "BULLISH"
            elif bear_bos.iloc[i]:
                market_structure.iloc[i] = "BEARISH"
            else:
                market_structure.iloc[i] = market_structure.iloc[i - 1] if i > 0 else "RANGING"
        return {
            "bull_bos": bull_bos,
            "bear_bos": bear_bos,
    ...
    def compute_signal_engine(
        ...
        df: pd.DataFrame,
        atr_series: pd.Series,
        ribbon_metrics: Dict[str, pd.Series],
        mpi_data: Dict[str, pd.Series],
        smc_flags: Dict[str, bool],
        wave_sequence: Dict[str, object],
        wave_exhaustion: Dict[str, pd.Series],
        chop_intensity: pd.Series,
        market_regime: pd.Series,
        htf_bias: pd.Series,
        structure: Dict[str, pd.Series],
        channel_metrics: Dict[str, pd.Series],
        relative_volume: pd.Series,
    ) -> Dict[str, pd.Series]:
        ribbon_top = ribbon_metrics["ribbon_top"]
        ribbon_bot = ribbon_metrics["ribbon_bot"]
        ribbon_mid = ribbon_metrics["ribbon_mid"]
        bullish_stack = ribbon_metrics["bullish_stack"]
        bearish_stack = ribbon_metrics["bearish_stack"]
        strict_bull_stack = ribbon_metrics["strict_bull_stack"]
        strict_bear_stack = ribbon_metrics["strict_bear_stack"]
        premium_zone = ribbon_metrics["premium_zone"]
        discount_zone = ribbon_metrics["discount_zone"]

        bars_above = 0
        bars_below = 0
        bars_in = 0
        phase_bull = pd.Series(False, index=df.index)
        phase_bear = pd.Series(False, index=df.index)
        in_ribbon_zone = pd.Series(False, index=df.index)
        ideal_long_zone = pd.Series(False, index=df.index)
        ideal_short_zone = pd.Series(False, index=df.index)
        pullback_from_above = pd.Series(False, index=df.index)
        pullback_from_below = pd.Series(False, index=df.index)
        for i in range(len(df)):
            if df['close'].iloc[i] > ribbon_top.iloc[i]:
                bars_above += 1
                bars_below = 0
                bars_in = 0
            elif df['close'].iloc[i] < ribbon_bot.iloc[i]:
                bars_below += 1
                bars_above = 0
                bars_in = 0
            else:
                bars_in += 1
                if bars_in > 5:
                    bars_above = max(0, bars_above - 1)
                    bars_below = max(0, bars_below - 1)
            if bars_above >= 3 or (i >= 5 and (phase_bull.iloc[i - 5] and 0 < bars_in <= 5)):
                phase_bull.iloc[i] = True
            if bars_below >= 3 or (i >= 5 and (phase_bear.iloc[i - 5] and 0 < bars_in <= 5)):
                phase_bear.iloc[i] = True
            in_ribbon_zone.iloc[i] = df['close'].iloc[i] >= ribbon_bot.iloc[i] - atr_series.iloc[i] * 0.2 and df['close'].iloc[i] <= ribbon_top.iloc[i] + atr_series.iloc[i] * 0.2
            ideal_long_zone.iloc[i] = df['close'].iloc[i] <= ribbon_mid.iloc[i] + atr_series.iloc[i] * 0.5 and df['close'].iloc[i] >= ribbon_bot.iloc[i] - atr_series.iloc[i] * 0.2
            ideal_short_zone.iloc[i] = df['close'].iloc[i] >= ribbon_mid.iloc[i] - atr_series.iloc[i] * 0.5 and df['close'].iloc[i] <= ribbon_top.iloc[i] + atr_series.iloc[i] * 0.2
            was_above = df['close'].iloc[max(0, i - 10): i + 1].max() > ribbon_top.iloc[i] + atr_series.iloc[i] * 0.5
            was_below = df['close'].iloc[max(0, i - 10): i + 1].min() < ribbon_bot.iloc[i] - atr_series.iloc[i] * 0.5
            pullback_from_above.iloc[i] = was_above and ribbon_bot.iloc[i] <= df['close'].iloc[i] <= ribbon_top.iloc[i]
            pullback_from_below.iloc[i] = was_below and ribbon_bot.iloc[i] <= df['close'].iloc[i] <= ribbon_top.iloc[i]

        wave_count = wave_sequence.get("wave_count", pd.Series(0, index=df.index))
        sequence_dir = wave_sequence.get("sequence_dir", pd.Series(0, index=df.index))
        is_wave_entry_zone = (wave_count == 2) | (wave_count == 4)
        is_wave5_caution = wave_count == 5
        wave_exhausted = wave_exhaustion.get("wave_exhausted", pd.Series(False, index=df.index))
        wave_dir = wave_exhaustion.get("current_wave_dir", pd.Series(0, index=df.index))
        wave_exhaustion_bullish = wave_exhausted & (wave_dir == -1)
        wave_exhaustion_bearish = wave_exhausted & (wave_dir == 1)

        bull_div = mpi_data["bull_divergence"]
        bear_div = mpi_data["bear_divergence"]
        mpi_turning_bull = mpi_data["mpi_turning_bull"]
        mpi_turning_bear = mpi_data["mpi_turning_bear"]
        mpi_bullish = mpi_data["mpi_bullish"]
        mpi_bearish = mpi_data["mpi_bearish"]
        mpi_supports_bull = mpi_data["mpi_supports_bull"]
        mpi_supports_bear = mpi_data["mpi_supports_bear"]

        bull_institutional = smc_flags["recent_bull_ob"] or smc_flags["recent_bull_fvg"] or smc_flags["in_bull_ob"]
        bear_institutional = smc_flags["recent_bear_ob"] or smc_flags["recent_bear_fvg"] or smc_flags["in_bear_ob"]
        channel_position = channel_metrics.get("channel_position", pd.Series(np.nan, index=df.index))
        at_channel_bottom = channel_position <= 20
        at_channel_top = channel_position >= 80
        near_channel_bottom = channel_position <= 35
        near_channel_top = channel_position >= 65

        confluence_long = pd.Series(0, index=df.index, dtype=float)
        confluence_short = pd.Series(0, index=df.index, dtype=float)
        confluence_long += np.where(strict_bull_stack, 3, np.where(bullish_stack, 2, 0))
        confluence_short += np.where(strict_bear_stack, 3, np.where(bearish_stack, 2, 0))
        confluence_long += np.where(structure["market_structure"] == "BULLISH", 2, 0)
        confluence_short += np.where(structure["market_structure"] == "BEARISH", 2, 0)
        confluence_long += np.where(ideal_long_zone & discount_zone, 3, np.where(ideal_long_zone | discount_zone, 2, np.where(in_ribbon_zone, 1, 0)))
        confluence_short += np.where(ideal_short_zone & premium_zone, 3, np.where(ideal_short_zone | premium_zone, 2, np.where(in_ribbon_zone, 1, 0)))
        confluence_long += np.where(mpi_turning_bull & mpi_bullish, 2, np.where(mpi_turning_bull | mpi_bullish, 1, 0))
        confluence_short += np.where(mpi_turning_bear & mpi_bearish, 2, np.where(mpi_turning_bear | mpi_bearish, 1, 0))
        confluence_long += np.where(bull_institutional & smc_flags["in_bull_ob"], 3, np.where(bull_institutional, 2, 0))
        confluence_short += np.where(bear_institutional & smc_flags["in_bear_ob"], 3, np.where(bear_institutional, 2, 0))
        confluence_long += np.where(smc_flags["in_bull_breaker"], self.config['i_breakerBoost'], 0)
        confluence_short += np.where(smc_flags["in_bear_breaker"], self.config['i_breakerBoost'], 0)
        confluence_long += np.where(bull_div, 2, 0)
        confluence_short += np.where(bear_div, 2, 0)
        confluence_long += np.where(wave_exhaustion_bullish, 2, 0)
        confluence_short += np.where(wave_exhaustion_bearish, 2, 0)
        confluence_long += np.where(at_channel_bottom, 2, np.where(near_channel_bottom, 1, 0))
        confluence_short += np.where(at_channel_top, 2, np.where(near_channel_top, 1, 0))

        confluence_long = confluence_long.clip(lower=0)
        confluence_short = confluence_short.clip(lower=0)
        max_confluence = 35.0
        score_long = (confluence_long / max_confluence) * 100
        score_short = (confluence_short / max_confluence) * 100
        grade_long = np.where(score_long >= 70, "A+", np.where(score_long >= 55, "A", np.where(score_long >= 40, "B", "C")))
        grade_short = np.where(score_short >= 70, "A+", np.where(score_short >= 55, "A", np.where(score_short >= 40, "B", "C")))

        min_grade_num = {"A+": 4, "A": 3, "B": 2, "C": 1}.get(self.config['i_minGrade'], 2)
        grade_num_map = {"A+": 4, "A": 3, "B": 2, "C": 1}
        long_grade_pass = pd.Series([grade_num_map[g] >= min_grade_num for g in grade_long], index=df.index)
        short_grade_pass = pd.Series([grade_num_map[g] >= min_grade_num for g in grade_short], index=df.index)

        is_chop_zone = chop_intensity >= self.config['i_chopThreshold']
        if self.config['i_enableSession']:
            asia = session_mask(df.index, self.config['i_asiaStart']) & self.config['i_allowAsia']
            london = session_mask(df.index, self.config['i_londonStart']) & self.config['i_allowLondon']
            ny = session_mask(df.index, self.config['i_nyStart']) & self.config['i_allowNY']
            session_allowed = asia | london | ny
        else:
            session_allowed = pd.Series(True, index=df.index)
        htf_aligned_long = (~self.config['i_enableHTF']) | (~self.config['i_requireHTFAlign']) | (htf_bias > 0)
        htf_aligned_short = (~self.config['i_enableHTF']) | (~self.config['i_requireHTFAlign']) | (htf_bias < 0)
        filters_pass_long = (~is_chop_zone) & session_allowed & htf_aligned_long
        filters_pass_short = (~is_chop_zone) & session_allowed & htf_aligned_short

        long_core = (
            (bullish_stack | phase_bull)
            & (ideal_long_zone | in_ribbon_zone)
            & (mpi_supports_bull | wave_exhaustion_bullish | bull_div)
            & long_grade_pass
            & (confluence_long >= self.config['i_minConfluence'])
            & filters_pass_long
        )
        short_core = (
            (bearish_stack | phase_bear)
            & (ideal_short_zone | in_ribbon_zone)
            & (mpi_supports_bear | wave_exhaustion_bearish | bear_div)
            & short_grade_pass
            & (confluence_short >= self.config['i_minConfluence'])
            & filters_pass_short
        )
        ...
        has_bull_wick = (lower_wick / total_range >= 0.25) & (lower_wick > atr_series * 0.15)
        has_bear_wick = (upper_wick / total_range >= 0.25) & (upper_wick > atr_series * 0.15)
        strong_bull_candle = (df['close'] > df['open']) & (body_size >= atr_series * 0.3)
        strong_bear_candle = (df['close'] < df['open']) & (body_size >= atr_series * 0.3)
        bull_candle_ok = (df['close'] > df['open']) | has_bull_wick
        bear_candle_ok = (df['close'] < df['open']) | has_bear_wick
        not_extended_long = df['close'] < ribbon_top + atr_series * 2.0
        not_extended_short = df['close'] > ribbon_bot - atr_series * 2.0
        volume_strong = relative_volume >= 1.0
        in_wave_entry_zone = (wave_count == 2) | (wave_count == 4)
        wave5_avoid_long = self.config['i_avoidWave5'] and is_wave5_caution
        wave5_avoid_short = self.config['i_avoidWave5'] and is_wave5_caution
        channel_ok_long = (~self.config['i_useChannelFilter']) | channel_position.isna() | (channel_position <= 60)
        channel_ok_short = (~self.config['i_useChannelFilter']) | channel_position.isna() | (channel_position >= 40)
        pullback_condition_long = (~self.config['i_requirePullback']) | pullback_from_above | in_ribbon_zone
        pullback_condition_short = (~self.config['i_requirePullback']) | pullback_from_below | in_ribbon_zone

        long_core = long_core & bull_candle_ok & not_extended_long & channel_ok_long & pullback_condition_long & (~wave5_avoid_long)
        short_core = short_core & bear_candle_ok & not_extended_short & channel_ok_short & pullback_condition_short & (~wave5_avoid_short)

        if self.config['i_useWaveEntryZones']:
            long_core = long_core | (in_wave_entry_zone & (bullish_stack | phase_bull) & bull_candle_ok & not_extended_long)
            short_core = short_core | (in_wave_entry_zone & (bearish_stack | phase_bear) & bear_candle_ok & not_extended_short)

        long_core = long_core & volume_strong
        short_core = short_core & volume_strong

        buy_signal = long_core & (~long_core.shift(1).fillna(False))
        sell_signal = short_core & (~short_core.shift(1).fillna(False))

        return {
            "confluence_long": confluence_long,
            "confluence_short": confluence_short,
            "grade_long": pd.Series(grade_long, index=df.index),
            "grade_short": pd.Series(grade_short, index=df.index),
            "buy_signal": buy_signal,
            "sell_signal": sell_signal,
            "long_core": long_core,
            "short_core": short_core,
            "is_wave_entry_zone": is_wave_entry_zone,
            "is_wave5_caution": is_wave5_caution,
        }
    ...
    def compute_channel_metrics(
        ...
        high_indices = [i for i, v in enumerate(swing_high) if not np.isnan(v)]
        low_indices = [i for i, v in enumerate(swing_low) if not np.isnan(v)]
        channel_position = pd.Series(np.nan, index=df.index)
        price_zone = pd.Series("—", index=df.index)
        upper = pd.Series(np.nan, index=df.index)
        lower = pd.Series(np.nan, index=df.index)
        center = pd.Series(np.nan, index=df.index)
        channel_width = pd.Series(np.nan, index=df.index)
        if len(high_indices) >= 2 and len(low_indices) >= 2:
            h1, h2 = high_indices[-2], high_indices[-1]
            l1, l2 = low_indices[-2], low_indices[-1]
            t = df.index.view("int64") / 1e9
            high_slope = (swing_high.iloc[h2] - swing_high.iloc[h1]) / (t[h2] - t[h1])
            low_slope = (swing_low.iloc[l2] - swing_low.iloc[l1]) / (t[l2] - t[l1])
            upper = pd.Series(swing_high.iloc[h2] + high_slope * (t - t[h2]), index=df.index)
            lower = pd.Series(swing_low.iloc[l2] + low_slope * (t - t[l2]), index=df.index)
            valid = (upper != lower)
            channel_position = pd.Series(
                np.where(valid, ((df['close'] - lower) / (upper - lower)) * 100, np.nan),
                index=df.index,
            ).clip(lower=0, upper=100)
            center = (upper + lower) / 2
            channel_width = (upper - lower).abs()
            price_zone = pd.Series(
                np.where(
                    channel_position >= 80,
                    "UPPER",
                    np.where(
                        ...
                        np.where(
                            channel_position >= 40,
                            "CENTER",
                            np.where(channel_position >= 20, "LOWER-MID", "LOWER"),
                        ),
                    ),
                ),
                index=df.index,
    ...
    def compute_wave_exhaustion(
        ...
        current_wave_start_price = pd.Series(np.nan, index=df.index)
        ...
        for i in range(len(df)):
            if not np.isnan(swing_high.iloc[i]) and last_swing_type != "HIGH":
                ...
                current_wave_start_price.iloc[i] = swing_high.iloc[i]
                wave_peak_momentum = 0.0
            elif not np.isnan(swing_low.iloc[i]) and last_swing_type != "LOW":
                last_swing_type = "LOW"
                current_wave_dir.iloc[i] = 1
                current_wave_start.iloc[i] = i - int(lookback.iloc[i])
                current_wave_start_price.iloc[i] = swing_low.iloc[i]
                wave_peak_momentum = 0.0
            else:
                ...
                current_wave_start_price.iloc[i] = current_wave_start_price.iloc[i - 1] if i > 0 else np.nan
        ...
        return {
            ...
            "current_wave_start_price": current_wave_start_price,
        }
    ...
    def compute_trade_management(
        ...
        tp3_series = pd.Series(np.nan, index=df.index)
        trade_state = pd.Series("FLAT", index=df.index)
        consec_loss_series = pd.Series(0, index=df.index)
        ...
        for i in range(len(df)):
            ...
            if signals["buy_signal"].iloc[i] and not in_long and not in_short:
                in_long = True
                entry_price = df['close'].iloc[i]
                if self.config['i_useSwingSL'] and not np.isnan(last_swing_low):
                    sl_price = last_swing_low
                else:
                    sl_price = entry_price - atr_series.iloc[i] * self.config['i_stopATR']
                tp1_price = entry_price + atr_series.iloc[i] * self.config['i_tp1Mult']
                tp2_price = entry_price + atr_series.iloc[i] * self.config['i_tp2Mult']
                tp3_price = entry_price + atr_series.iloc[i] * self.config['i_tp3Mult']
                t1_hit = t2_hit = t3_hit = stop_hit = False
            elif signals["sell_signal"].iloc[i] and not in_long and not in_short:
                in_short = True
                entry_price = df['close'].iloc[i]
                if self.config['i_useSwingSL'] and not np.isnan(last_swing_high):
            ...
            if in_long:
                if df['low'].iloc[i] <= sl_price:
                    ...
                    in_long = False
                    consec_losses += 1
                if df['high'].iloc[i] >= tp1_price:
                    t1_hit = True
                if df['high'].iloc[i] >= tp2_price:
                    t2_hit = True
                if df['high'].iloc[i] >= tp3_price:
                    t3_hit = True
                    in_long = False
                    consec_losses = 0
            if in_short:
                ...
                if df['low'].iloc[i] <= tp1_price:
                    t1_hit = True
                if df['low'].iloc[i] <= tp2_price:
                    t2_hit = True
                if df['low'].iloc[i] <= tp3_price:
                    t3_hit = True
                    in_short = False
                    consec_losses = 0

            if in_long:
                trade_state.iloc[i] = "LONG"
            elif in_short:
                trade_state.iloc[i] = "SHORT"
            else:
                trade_state.iloc[i] = "FLAT"

            entry_series.iloc[i] = entry_price
            sl_series.iloc[i] = sl_price
            tp1_series.iloc[i] = tp1_price
            tp2_series.iloc[i] = tp2_price
            tp3_series.iloc[i] = tp3_price
            consec_loss_series.iloc[i] = consec_losses
        ...
        return {
            ...
            "tp3": tp3_series,
            "state": trade_state,
            "consec_losses": consec_loss_series,
        }
    ...
    def run(self, df: pd.DataFrame) -> Dict[str, object]:
        ...
        ribbon_metrics = self.compute_ribbon_metrics(df, atr_series)
        ribbon = ribbon_metrics["ribbon"]
        ...
        market_regime = pd.Series("WEAK_TREND", index=df.index)
        market_regime = np.where((adx_val >= 30) & (choppiness <= 61.8), "STRONG_TREND", market_regime)
        market_regime = np.where((adx_val >= self.config['i_adxThreshold']) & (chop_intensity < 40), "TRENDING", market_regime)
        market_regime = np.where((chop_intensity >= 60) & (range_compression < 0.6), "COMPRESSION", market_regime)
        market_regime = np.where(chop_intensity >= 50, "RANGING", market_regime)
        market_regime = np.where(chop_intensity >= 35, "TRANSITIONING", market_regime)
        ...
        structure = self.compute_structure(df, swing_high, swing_low, relative_volume)
        channel_metrics = self.compute_channel_metrics(df, swing_high, swing_low)
        signal_engine = self.compute_signal_engine(
            df,
            atr_series,
            ribbon_metrics,
            mpi_data,
        ...
        measured_move = pd.Series(np.nan, index=df.index)
        in_active_sequence = wave_sequence.get("in_active_sequence", pd.Series(False, index=df.index))
        wave_count = wave_sequence.get("wave_count", pd.Series(0, index=df.index))
        sequence_dir = wave_sequence.get("sequence_dir", pd.Series(0, index=df.index))
        wave1_mag = wave_sequence.get("wave1_mag")
        wave3_mag = wave_sequence.get("wave3_mag")
        w4_end_price = wave_sequence.get("w4_end_price", pd.Series(np.nan, index=df.index))
        if self.config['i_showWaveProjections'] and in_active_sequence.iloc[-1] and wave_count.iloc[-1] >= 3:
            if wave1_mag is not None and not np.isnan(wave1_mag) and not np.isnan(w4_end_price.iloc[-1]):
                avg_mag = (wave1_mag + (wave3_mag if wave3_mag is not None and not np.isnan(wave3_mag) else wave1_mag)) / 2
                if sequence_dir.iloc[-1] == 1:
                    measured_move.iloc[-1] = w4_end_price.iloc[-1] + avg_mag * 0.786
                elif sequence_dir.iloc[-1] == -1:
                    measured_move.iloc[-1] = w4_end_price.iloc[-1] - avg_mag * 0.786

        if self.config['i_showProjections']:
            current_wave_dir = wave_exhaustion.get("current_wave_dir", pd.Series(0, index=df.index))
            current_wave_start_price = wave_exhaustion.get("current_wave_start_price", pd.Series(np.nan, index=df.index))
            angular_momentum = angular_data["angular_momentum"]
            base_projection = atr_series.iloc[-1] * 2.0 if not np.isnan(atr_series.iloc[-1]) else np.nan
            if wave1_mag is not None and not np.isnan(wave1_mag):
                base_projection = wave1_mag
                if wave3_mag is not None and not np.isnan(wave3_mag):
                    avg_wave_mag = (wave1_mag + wave3_mag) / 2.0
                    last_wave_ratio = wave3_mag / wave1_mag if wave1_mag > 0 else 1.0
                    base_projection = avg_wave_mag
                    if last_wave_ratio > 1.3:
                        base_projection *= 1.2
                    elif last_wave_ratio < 0.7:
                        base_projection *= 0.8
            momentum_adjust = 1.0 + (angular_momentum.iloc[-1] / 200.0 if not np.isnan(angular_momentum.iloc[-1]) else 0.0)
            momentum_adjust = max(0.7, min(1.5, momentum_adjust))
            if not np.isnan(base_projection) and not np.isnan(current_wave_start_price.iloc[-1]) and current_wave_dir.iloc[-1] != 0:
                base_projection *= momentum_adjust
                if current_wave_dir.iloc[-1] == 1:
                    targets = [
                        current_wave_start_price.iloc[-1] + (base_projection * 0.618),
                        current_wave_start_price.iloc[-1] + base_projection,
                        current_wave_start_price.iloc[-1] + (base_projection * 1.618),
                    ]
                else:
                    targets = [
                        current_wave_start_price.iloc[-1] - (base_projection * 0.618),
                ...
                projections = [(len(df) - 1, target) for target in targets]
        trades = self.compute_trade_management(df, atr_series, signal_engine, swing_high, swing_low)
        return {
            "atr": atr_series,
            "lookback": lookback,
...
def main() -> None:
    if CONFIG.get("i_scanEnabled"):
        scan_results = scan_binance_usdm(CONFIG)
        print("Binance USDT-M Futures scan:")
        print("Buy signals:", ", ".join(scan_results["buy"]) or "None")
        print("Sell signals:", ", ".join(scan_results["sell"]) or "None")
        return
    df = fetch_data()
    indicator = WaveDynamicsIndicator(CONFIG)
    results = indicator.run(df)
    if CONFIG.get("i_plotEnabled"):
